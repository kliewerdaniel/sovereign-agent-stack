...
class QuantKnowledgeCompiler:
    ...
    def compile_policies(self, policy_dir: Path) -> dict[str, dict]:
        ...
        for md_file in sorted(policy_dir.glob("*.md")):
            ...
            for node in result.nodes:
                # node.properties is a dict per the SAS knowledge layer
                props = dict(node.properties) if hasattr(node, 'properties') else {}
                props["content"] = content
                nodes[node.label] = props
    ...
    def compile(self, source: Path) -> dict:
        ...
        graph = self.compiler.compile(source)
